...
export default function ContactPage() {
  useEffect(() => {
    document.title = 'Contact — Purpose Owned'
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://formspree.io/f/mjglzgwl', {
        ...
        body: JSON.stringify({ name: formName, email: formEmail, message: formMessage, form_type: 'purposeowned_inquiry' }),
      })
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-[#f5f5f7] text-gray-800 font-sans flex flex-col">
      <main className="flex-1 max-w-xl mx-auto w-full px-5 py-8 flex flex-col">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm w-full px-6 py-8">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 text-center mb-2">Contact</h1>
          ...
          {submitted ? (
            <div className="text-center py-6">
              ...
              <Link to="/" className="text-sm text-[#1a6847] hover:text-[#145236] transition-colors font-medium">&larr; Back home</Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={formName} onChange={e => setFormName(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#1a6847] transition-colors"
              ...
              <div>
                ...
                <textarea rows={5} value={formMessage} onChange={e => setFormMessage(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#1a6847] transition-colors resize-none"
              ...
              <div className="bg-[#f5f5f7] rounded-xl px-4 py-3">
                ...
                <input type="text" value={captchaAnswer} onChange={e => setCaptchaAnswer(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#1a6847] transition-colors bg-white"
                  placeholder="Answer" />
              </div>
              {formError && <p className="text-red-600 text-sm">{formError}</p>}
              <button type="submit"
                className="w-full bg-[#1a6847] text-white font-semibold py-3 rounded-lg text-sm hover:bg-[#145236] transition-colors flex items-center justify-center gap-2">
                <Send size={15} /> Send
              </button>
            </form>
          )}
        ...
        <div className="mt-3 flex justify-center gap-4">
          ...
          <Link to="/about" className="text-sm text-[#1a6847] hover:text-[#145236] transition-colors font-medium">
            About & FAQ &rarr;
          </Link>
        </div>
      </main>
